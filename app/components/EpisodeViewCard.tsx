...
export default function EpisodeViewCard({tmdb_id, poster, title, date,season_number, episode_number, current_ep}:{tmdb_id:number, poster:string, title:string, date:string, season_number:number, episode_number:number, current_ep:number}):JSX.Element{
...
// console.log(episode_number==current_ep)
    ...
    return(
        <div className={`rounded-2xl ${episode_number==current_ep?"bg-amber-600":null} border-4 border-gray-500 px-2 py-2 text-white shadow-2xl hover:transition-transform transition-discrete duration-300 ease-in-out delay-150 hover:scale-105`}>
