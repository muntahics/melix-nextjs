...
export default function ActorsDetails({birthday, deathday, id, name, also_known_as, place_of_birth, profile_path, biography, known_for}:Props):JSX.Element{
        ...
        <main className="lg:flex lg:flex-col lg:justify-center lg:items-center">
            <div className="flex flex-col md:flex-row m-6 lg:max-w-360">
                ...
                <div className="flex flex-col justify-start max-sm:gap-2 gap-4 md:ml-6">
                    ...
                    <span className="block text-gray-300">Known As: {also_known_as}</span> 
                    {deathday && `<span className="block text-gray-300">Died: ${FormatDate(deathday)}</span>`}
                    <span className="block text-gray-300">Born: {FormatDate(birthday)}</span> 
                    <span className="block text-gray-300">Age: {currentYear - Number(birthday.slice(0,4))}</span> 
                     
                    
                    <span className="block text-gray-300">Birthplace: {place_of_birth}</span> 
                    ...
                    <p className="inline text-gray-300"><span className="text-lg text-gray-300">Plot: </span>{biography}</p>
